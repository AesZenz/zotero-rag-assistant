...
- **Pytest test suite** (`tests/`): covers all eight core modules plus a full-pipeline integration test (`@pytest.mark.integration`); 53 tests, all passing; `sentence_transformers` and `anthropic` clients mocked throughout — no model downloads or API calls in unit tests; `OMP_NUM_THREADS=1` set in `conftest.py`; manually reviewed and verified
...
## Next Steps (ordered)
1. **Install and test RAGAS** — add `ragas` to `pixi.toml` pypi-dependencies, run `pixi run evaluate --full`, verify the RAGAS metrics path in `ragas_evaluator.py` produces valid scores
