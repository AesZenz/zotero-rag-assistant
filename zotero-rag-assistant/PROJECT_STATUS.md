...
- **Test scripts** (`scripts/test_pdf_parser.py`, `test_chunker.py`, `test_embedder.py`, `test_vector_store.py`): Ad-hoc smoke tests for each stage — ingestion confirmed working; vector store test covers full parse→chunk→filter→embed→index→search→save→load round-trip
- **PDF library**: 1.3GB, ~600 psychology/neuroscience/AI papers exported from Zotero (with RDF metadata)

## Current State
The ingestion + retrieval layers are complete. A PDF can be parsed, chunked into 512-token overlapping chunks, noise-filtered (references/affiliations/headers removed), embedded with local sentence-transformers, indexed in FAISS, searched by cosine similarity, and saved/loaded from disk — confirmed on `flynn effect visospatial meta analysis.pdf`. There is still no way to generate answers: the generation and evaluation modules are absent, and the main entry-point scripts (`ingest_papers.py`, `query_assistant.py`, `run_evaluation.py`) don't exist.
...
- **Missing entry-point scripts**: `scripts/ingest_papers.py`, `scripts/query_assistant.py`, `scripts/run_evaluation.py` are referenced in `pixi.toml` tasks but don't exist — `pixi run ingest` will fail
...
- **Unused heavy dependencies**: `langchain`, `langchain-community`, `chromadb`, `anthropic`, `rich`, `click`, `pandas`, `scikit-learn`, `nltk` all installed but not imported anywhere
- **`pdfplumber` installed but unused**: PyMuPDF was chosen but pdfplumber remains as dead weight

## Next Steps (ordered)
1. **Build `scripts/ingest_papers.py`** — walk the PDF library, run the full ingestion pipeline (parse→chunk→filter→embed), persist embeddings to a FAISS index on disk
3. **Implement Claude generation layer** (`src/generation/`) — prompt template, Anthropic API call, token limit enforcement using `MAX_TOKENS_PER_RESPONSE`
4. **Build `scripts/query_assistant.py`** — end-to-end: user query → embed query → FAISS search → Claude generation → answer with citations
5. **Add Pydantic settings** — centralize all env var reads into one `Settings` class with validation
6. **Implement evaluation** (`src/evaluation/`) — question generation + retrieval/answer quality metrics using the existing `scikit-learn`/`nltk` deps
7. **Write pytest test suite** — unit tests for parser, chunker, embedder, vector store; integration test for full pipeline on known PDF

## Concepts Learned So Far
- **RAG pipeline**: Understand the full 4-stage flow (ingest → embed → retrieve → generate); have built stage 1 completely; stages 2-4 are architecture-planned but not coded
...
- Claude API test calls: ~$0 (API not integrated yet)
- GPU rental: $0
